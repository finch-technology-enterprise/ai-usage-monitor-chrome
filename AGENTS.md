...
- Permissions are intentionally narrow: `storage`, `cookies`, plus `host_permissions` for the nine providers' hosts — `opencode.ai`, `claude.ai`, `chatgpt.com`, `copilot.github.com` and `api.githubcopilot.com`, `gemini.google.com`, `cursor.com` and `curl.cursorapi.com`, `www.perplexity.ai`, `chat.mistral.ai`, `grok.com` (see `manifest.json`). Adding a provider means adding its hosts to `host_permissions` and, for session-based providers, a `relay-*.js` content script. Never add `<all_urls>`.
